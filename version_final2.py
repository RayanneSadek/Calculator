    ...
    if not history:
                ...
                match history_option:
                    ...
                    case "E":
                        print("═══════════════════════════════════════════════════════")
                        print("                   ══ HISTORIQUE ══")
                        print("ici on mettra la fonction de Rayanne")
                        for i, op in enumerate(history, start=1):
                            print(f"{i}. {op['time']} | {op['operation']}")
                        print("quelle opération souhaitez-vous effacer ?")
                        print("═══════════════════════════════════════════════════════")
                    case "D":
                        delete_choice = input(f"Warning: Voulez-vous vraiment effacer l'historique ? \nTapez oui pour confirmer ou n'importe quelle touche pour sortir: \n").strip().upper()
        ...
        while True:
            ...
            if second_operation == 'oui':
                ...
                if second_operator == "sqr":
                    ...
                    second_result = mute_integer(second_result)
                    print(f"Le résultat final de sqrt({result}) est : {second_result}")
                    history.append({"time": get_time(), "operation": f"{result} {second_operator} = {second_result}"})
                    ...
                    if second_result is not None:
                        second_result = mute_integer(second_result)
                        print(f"Le résultat final de {result} {second_operator} {c} est : {second_result}")
                        history.append({"time": get_time(), "operation": f"{result} {second_operator} {c} = {second_result}"})
